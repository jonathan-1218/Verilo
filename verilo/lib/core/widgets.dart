...
/// Wraps a bottom-nav tab root so the Android back gesture returns to the
/// dashboard instead of quitting the app. Tab switches use `context.go`, which
/// replaces the stack, so without this there is nothing left to pop.
/// The dashboard itself stays unwrapped — back there exits, as users expect.
class TabBackScope extends StatelessWidget {
  const TabBackScope({super.key, required this.child});
  final Widget child;
    ...
    child: AnimatedScale(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: _pressed ? AppColors.copperDark : AppColors.copperMid,
          borderRadius: BorderRadius.circular(14),
          boxShadow: const [
            BoxShadow(
              color: Color(0x4DB87040),
              blurRadius: 16,
              offset: Offset(0, 4),
            ),
          ],
...
class VeriloLogo extends StatelessWidget {
  ...
  Widget build(BuildContext context) => RichText(
    text: TextSpan(
      children: [
        // brand lockup: 'veri' neutral, 'lo' copper (see exports/)
        TextSpan(
          text: 'veri',
          style: AppText.spaceGrotesk(
            size: size,
            ...
            Expanded(
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: hint,
                  hintStyle: AppText.spaceGrotesk(
                    size: 14,
...
class StatusChip extends StatelessWidget {
  const StatusChip({
    super.key,
    required this.label,
    this.color = AppColors.copperMid,
    this.bg,
  });
  final String label;
  final Color color;
...
class BottomNav extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    // bottom SafeArea keeps the bar above the gesture-nav home indicator
    child: SafeArea(
      top: false,
      child: SizedBox(
        height: 56,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Icon(
                      ...
                      size: 20,
                      color: active ? AppColors.copperMid : AppColors.textMuted,
                    ),
